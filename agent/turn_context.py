...
def build_turn_context(
    ...
    restoring_prompt = agent._cached_system_prompt is None and bool(conversation_history)
    if agent._cached_system_prompt is None:
        restore_or_build_system_prompt(agent, system_message, conversation_history)
    if getattr(agent, "_memory_refresh_on_turn", False) is True and agent._memory_store is not None:
        store = agent._memory_store
        targets = tuple(target for target in ("memory", "user") if store.target_enabled(target))
        previous_memory = tuple(store.format_for_system_prompt(target) for target in targets)
        store.load_from_disk()
        current_memory = tuple(store.format_for_system_prompt(target) for target in targets)
        # A reconstructed store already contains today's files, not the saved prompt's
        # snapshot. Refresh once on restore; resident turns rebuild only on rendered change.
        # Keep the native invalidation boundary, including its plugin refresh, when needed.
        if restoring_prompt or current_memory != previous_memory:
